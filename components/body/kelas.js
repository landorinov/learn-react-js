...
export default function Kelas() {
    ...
    const getMataKuliahs = async () => {
        const host = `${url}/v1/mata-kuliah/list?page=0&size=100&nama=`
        const res = await fetch(host, { headers }).catch(err => console.error(err))
        if (res?.ok) {
            const newData = await res.json()
    ...
    const showAction = (e, i) => {
        ...
        if (e == 'add') {
            ...
            setInputs({
                ...
                semester: Number(i.semester?.id),
                mataKuliah: Number(i.mataKuliah?.id),
                lingkup: Number(i.lingkup?.id),
                mode: Number(i.mode?.id),
                                    ...
                                    <tbody>
                                        {
                                            kelass.map((item, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>{item.lingkup.nama}</td>
                                                    <td>{item.mataKuliah.jenisMatkul.nama}</td>
